          ...
          {swimmers.map(swimmer => (
            <React.Fragment key={swimmer._id}>
              <tr>
                ...
                <td>
                  <button className="primary-button" style={{padding: '0.2rem 0.7rem', fontSize: '0.95rem'}} onClick={() => setShowLog(showLog === swimmer._id ? null : swimmer._id)}>
                      ...
                      <table style={{marginTop: 6, minWidth: 320}}>
                        ...
                        <tbody>
                          {swimmer.payment_log && swimmer.payment_log.map((entry, idx) => (
                            <tr key={idx}>
                              <td>{entry.month}</td>
                              <td style={{color: entry.status === 'Paid' ? 'green' : 'red', fontWeight: 500}}>{entry.status}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
