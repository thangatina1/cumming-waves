import os
# Ensure latest stripe package is installed and imported
import stripe
# API to create a Stripe PaymentIntent and return client secret
...
STRIPE_SECRET_KEY = os.environ.get("STRIPE_SECRET_KEY", "sk_test_...replace_with_your_key...")
stripe.api_key = STRIPE_SECRET_KEY

@app.post("/create-payment-intent")
async def create_payment_intent(amount: float = Body(...)):
    try:
        ...
        intent = stripe.PaymentIntent.create(
            ...
            description="Cumming Waves Swim Team Payment"
        )
...
async def process_card_payment(
    ...
    """
    Validate card details and process payment (no storage).

    This endpoint validates card details and simulates payment processing. In
    production, integrate with a payment gateway.

    Args:
        card_number: The 13-19 digit card number to be validated.
        name: The cardholder's name.
        expiry: The card's expiry date in MM/YY format.
        cvc: The 3-4 digit CVC number on the back of the card.
        amount: The amount to be charged in the payment.

    Returns:
        A JSON response with a 'message' and 'amount' key. The message is
        'Payment processed successfully! (Demo)' and the amount is the same
        as the input amount. If any of the validation steps fail, a 400 error
